...
namespace RemoteSensingAlgorithms
{
    ...
    {
        return (nir - red) / (red + nir + 1e-9); // 加极小值防止除零
    };
    ...
    {
        return (1 + L) * (nir - red) / (nir + red + L + 1e-9);
    };
    ...
    {
        if (k <= 0 || data.empty())
            throw std::invalid_argument("kMeansClustering: k must be > 0 and data not empty");
                                         ...
                                         const std::vector<std::vector<double>> &kernel)
    {
        if (image.empty() || kernel.empty() || kernel[0].empty())
            throw std::invalid_argument("convolve: non-empty image and kernel required.");
        ...
        if (imgRows < kRows || imgCols < kCols)
            throw std::invalid_argument("convolve: kernel larger than image.");

        size_t outRows = imgRows - kRows + 1;
        size_t outCols = imgCols - kCols + 1;
        std::vector<std::vector<T>> output(outRows, std::vector<T>(outCols));
