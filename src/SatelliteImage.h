...
private:
    ...
    vector<vector<Pixel<double>>> data; // 影像数据矩阵
    vector<double> bandStatistics;      // 波段统计信息
...
public:
    // 构造函数
    SatelliteImage(const string& id, const string& name, const string& path,
                   int w, int h, int b, const string& sensor, double time = 0);
    ...
    string getType() const override { return "SatelliteImage"; }
    ...
    // 下标运算符（访问行）
    vector<Pixel<double>>& operator[](int row);
    const vector<Pixel<double>>& operator[](int row) const;
    
    // 类型转换运算符
    operator double() const { return cloudCover; }  // 转换为云量
    operator string() const;                         // 转换为摘要字符串
    
    // 子图操作
    SatelliteImage getSubImage(int x, int y, int w, int h) const;
    
    // 波段操作
    vector<double> getBandValues(int band) const;  // 0:R,1:G,2:B,3:NIR,4:Thermal
    SatelliteImage extractBand(int band) const;
    
    // 遥感指数计算（返回二维数组）
    vector<vector<double>> calculateNDVI() const;
    vector<vector<double>> calculateNDWI() const;
    vector<vector<double>> calculateSAVI(double L = 0.5) const;
    vector<vector<double>> calculateTemperature() const;
    ...
    // 静态工厂方法
    static SatelliteImage createRandomImage(const string& id, int w, int h);
    static SatelliteImage createConstantImage(const string& id, int w, int h, 
